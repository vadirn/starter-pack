...
export default function Playground() {
  const { appState, controller } = useContext(AppContext);

  useEffect(
    () => {
      window.scrollTo(0, 0);
    },
    [appState.Playground.component]
  );

  const toggleGrid = useCallback(
    evt => {
      const { checked } = evt.target;
      controller.toggleGrid({ checked, page: appState.page });
    },
    [appState.Playground.displayGrid]
  );

  let item;
  for (const group of components) {
    for (const _item of group.items) {
      if (_item.key === appState.Playground.component) {
        item = _item;
      }
    }
  }
  ...
  return (
    <div className={s.container}>
      <Toolbar
        ...
        right={<SwitchButton checked={!!appState.Playground.displayGrid} onChange={toggleGrid} left="Grid" />}
      />
      <div className="p-s-l">
        <Navigation page={appState.page} components={components} component={appState.Playground.component} />
      </div>
      <div className="relative p-s-r">
        <div className={c({ 'bg-grid z999': appState.Playground.displayGrid }, 'absolute', s.grid)} />
